...
public class ScreenEffect implements IScreenEffect {
	...
	public ScreenEffect(String fxTexture, int cols,
		...
		super();
		this.fxTexture = new ResourceLocation(SagerFX.MODID,fxTexture);
		this.cols = cols;
		this.rows = rows;
...
//	/* (non-Javadoc)
//	 * @see safx.client.render.fx.IScreenEffect#doRender(float, float, float, float, float, boolean)
//	 */
//	@Override
...
//	}
	...
	/* (non-Javadoc)
	 * @see safx.client.render.fx.IScreenEffect#doRender(float, float, float, float, float, float, float, float, boolean)
	 */
	@Override
	public void doRender(float progress, float offsetX, float offsetY, float offsetZ, float scale, float rot_x, float rot_y, float rot_z, boolean is3p) {
		...
		if (currentFrame < numSprites) {
			...
			SARenderHelper.enableBlendMode(type);
...
//			
           ...
           //SARenderHelper.disableFXLighting();
	        SARenderHelper.disableBlendMode(type); 
		}
	}
