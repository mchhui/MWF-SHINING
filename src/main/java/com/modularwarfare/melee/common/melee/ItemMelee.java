...
public class ItemMelee extends BaseItem {
    ...
    public Multimap<String, AttributeModifier> getItemAttributeModifiers(EntityEquipmentSlot equipmentSlot) {
        ...
        if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
            ...
            // AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Weapon modifier",
            // this.type.damage, 0));
            multimap.put(SharedMonsterAttributes.ATTACK_SPEED.getName(),
                    new AttributeModifier(ATTACK_SPEED_MODIFIER, "Weapon modifier", this.type.attackspeed, 0));
        }
    ...
    public boolean onBlockStartBreak(ItemStack itemstack, BlockPos pos, EntityPlayer player) {
        ...
        if (!world.isRemote) {
            if (!type.destroyBlocks) {
                // Client will still render block break if player is in creative so update block
                // state
