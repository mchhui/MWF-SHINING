...
public class ActionPlaybackMelee {
    ...
    public void updateTime(int currentAnim, double alpha) {
        if (config.meleeAnimations.get(action) == null) {
            return;
        }
        if (config.meleeAnimations.get(action).get(currentAnim) == null) {
            return;
        }
        double startTime = config.meleeAnimations.get(action).get(currentAnim).getStartTime(config.FPS);
        double endTime = config.meleeAnimations.get(action).get(currentAnim).getEndTime(config.FPS);
