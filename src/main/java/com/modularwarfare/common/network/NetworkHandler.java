...
public class NetworkHandler extends MessageToMessageCodec<FMLProxyPacket, PacketBase> {
     ...
     */
    private final ConcurrentLinkedQueue<PacketBase> receivedPacketsClient = new ConcurrentLinkedQueue<PacketBase>();
    private final ConcurrentHashMap<UUID, ConcurrentLinkedQueue<PacketBase>> receivedPacketsServer = new ConcurrentHashMap<UUID, ConcurrentLinkedQueue<PacketBase>>();
    ...
    protected void encode(ChannelHandlerContext ctx, PacketBase msg, List<Object> out) throws Exception {
        try {
            ...
            FMLProxyPacket proxyPacket = new FMLProxyPacket(new PacketBuffer(encodedData), ctx.channel().attr(NetworkRegistry.FML_CHANNEL).get());
    ...
    public void initialise() {
        ...
        registerPacket(PacketPlaySoundLinear.class);
...
//        registerPacket(PacketExpShot.class);
        ...
        registerPacket(PacketAimPoseSync.class);
        registerPacket(PacketAimPoseSyncClient.class);
        registerPacket(PacketGrenadeThrow.class);
        registerPacket(PacketGrenadeConsume.class);

        registerPacket(PacketTeslaTrail.class);
