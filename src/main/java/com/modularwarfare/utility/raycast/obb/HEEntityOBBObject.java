...
public class HEEntityOBBObject extends OBBModelObject {
    ...
    private void createBoneForNode(UPCNodeData nodeData) {
        ...
        bone.oirign = new com.modularwarfare.utility.vector.Vector3f(0, 0, 0);
        ...
        box.size = new com.modularwarfare.utility.vector.Vector3f(
            1.0f * scaleFactor,
            1.0f * scaleFactor,
            1.0f * scaleFactor
        );
    ...
    private static final int DEBUG_INTERVAL = 60; // 每60帧输出一次（约1秒）
     ...
     public void updatePose(float partialTicks) {
         ...
         // 【第一步】构建实体的世界变换矩阵
         com.modularwarfare.utility.vector.Matrix4f entityWorldMatrix = new com.modularwarfare.utility.vector.Matrix4f();
         entityWorldMatrix.setIdentity();
         entityWorldMatrix.translate(new com.modularwarfare.utility.vector.Vector3f(entityX, entityY, entityZ));
         entityWorldMatrix.rotate((float)Math.toRadians(entityYaw), new com.modularwarfare.utility.vector.Vector3f(0, -1, 0));
         ...
         for (UPCNodeData nodeData : upcNodeDataMap.values()) {
             ...
             // 将HE引擎的局部矩阵转换为MW矩阵
             com.modularwarfare.utility.vector.Matrix4f localMatrix = convertJOMLToMW(nodeData.worldMatrix);
             
             // 【关键】手动计算世界矩阵 = 实体世界矩阵 * 局部矩阵
             com.modularwarfare.utility.vector.Matrix4f worldMatrix = 
                 com.modularwarfare.utility.vector.Matrix4f.mul(entityWorldMatrix, localMatrix, null);
             ...
             for (OBBModelBox box : boxes) {
                 if (boneBinding.get(box) == bone) {
                     box.compute(new com.modularwarfare.utility.vector.Matrix4f(worldMatrix).translate(bone.oirign));
                 }
             }
     ...
     */
    private com.modularwarfare.utility.vector.Matrix4f convertJOMLToMW(Matrix4f jomlMatrix) {
        com.modularwarfare.utility.vector.Matrix4f mwMatrix = new com.modularwarfare.utility.vector.Matrix4f();
    ...
    private void renderOBBWireframe(OBBModelBox box) {
        // OBB的8个顶点
        com.modularwarfare.utility.vector.Vector3f center = box.center;
        com.modularwarfare.utility.vector.Vector3f[] axes = {
            box.axis.x,
            box.axis.y,
            box.axis.z
        };
        ...
        // 计算8个顶点
        com.modularwarfare.utility.vector.Vector3f[] vertices = new com.modularwarfare.utility.vector.Vector3f[8];
        for (int i = 0; i < 8; i++) {
            float sx = (i & 1) == 0 ? -1 : 1;
            float sy = (i & 2) == 0 ? -1 : 1;
            float sz = (i & 4) == 0 ? -1 : 1;
            
            vertices[i] = new com.modularwarfare.utility.vector.Vector3f(
     ...
     * 绘制一条线
     */
    private void drawLine(com.modularwarfare.utility.vector.Vector3f start, com.modularwarfare.utility.vector.Vector3f end) {
