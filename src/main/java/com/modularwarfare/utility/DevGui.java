...
public class DevGui extends Gui {
    ...
    public DevGui(Minecraft mc, ItemStack itemStack, ItemGun itemGun, RenderGunStatic renderGun, AnimStateMachine anim) {
        ...
        if(gunType.animationType == WeaponAnimationType.ENHANCED) {
            ModelEnhancedGun gunModel = (ModelEnhancedGun) gunType.enhancedModel;
            renderDevGuiEnhanced(mc, itemStack, gunType, gunModel);
        } else {
            ModelGun gunModel = (ModelGun) gunType.model;
            renderDevGuiBasic(mc, itemStack, gunType, gunModel, anim);
        }
    }

    private void renderDevGuiEnhanced(Minecraft mc, ItemStack itemStack, GunType gunType, ModelEnhancedGun gunModel) {
        ...
        if (mc.player.inventory.armorItemInSlot(3) != null && mc.player.inventory.armorItemInSlot(3).getItem() == Items.GOLDEN_HELMET) {
            ...
            String modelScale = "Model Scale - " + Float.toString(1.0f);
            ...
            String animationType = "Animation Type - " + gunType.animationType.toString();
            ...
            GL11.glPushMatrix();
            {
                ...
                mc.fontRenderer.drawString(animationType, (width), (height) + 152, Integer.parseInt("FFFFFF", 16), false);
            }
            GL11.glPopMatrix();
        ...
        if (mc.player.inventory.armorItemInSlot(1) != null && mc.player.inventory.armorItemInSlot(1).getItem() == Items.GOLDEN_LEGGINGS) {
            AnimationController controller = AnimationController.getController(mc.player, gunModel.config);
            boolean hasCameraNode = gunModel.model.geoModel.nodes.get("mwf_camera") != null;
            
            String currentAnim = "Current Animation - " + (controller.getPlayingAnimation() != null ? controller.getPlayingAnimation().toString() : "None");
            String animSpeed = "Animation Speed - " + String.format("%.2f", controller.customAnimationSpeed);
            String animTimeRange = String.format("Animation Time Range - %.2f to %.2f", controller.startTime, controller.endTime);
            String hasCamera = "Has Camera Node - " + hasCameraNode;
            
            String cameraPos = "Camera Node Position - N/A";
            String cameraRot = "Camera Node Rotation - N/A";
            String appliedPos = "Applied Camera Position - N/A";
            String appliedRot = "Applied Camera Rotation - N/A";
            
            if(hasCameraNode) {
                Vector3f cam_pos = RenderGunEnhanced.mwf_camera_pos;
                AxisAngle4d cam_rot = RenderGunEnhanced.mwf_camera_rot;
                Vector3f cam_begin_pos = gunModel.model.geoModel.nodes.get("mwf_camera").pos;
                
                cameraPos = String.format("Camera Node Position - X:%.2f Y:%.2f Z:%.2f", cam_begin_pos.x, cam_begin_pos.y, cam_begin_pos.z);
                cameraRot = String.format("Camera Node Rotation - Angle:%.2f X:%.2f Y:%.2f Z:%.2f", 
                    Math.toDegrees(cam_rot.angle), cam_rot.x, cam_rot.y, cam_rot.z);
                appliedPos = String.format("Applied Camera Position - X:%.2f Y:%.2f Z:%.2f", cam_pos.x, cam_pos.y, cam_pos.z);
                appliedRot = String.format("Applied Camera Rotation - Angle:%.2f X:%.2f Y:%.2f Z:%.2f",
                    Math.toDegrees(cam_rot.angle), -cam_rot.x, -cam_rot.y, -cam_rot.z);
            }
            ...
            {
                GL11.glScalef(textScale, textScale, textScale);
                mc.fontRenderer.drawString("Animation Debug;", (width), (height) + 176, Integer.parseInt("FF0000", 16), false);
                mc.fontRenderer.drawString(currentAnim, (width), (height) + 184, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(animSpeed, (width), (height) + 192, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(animTimeRange, (width), (height) + 200, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(hasCamera, (width), (height) + 208, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(cameraPos, (width), (height) + 216, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(cameraRot, (width), (height) + 224, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(appliedPos, (width), (height) + 232, Integer.parseInt("FFFFFF", 16), false);
                mc.fontRenderer.drawString(appliedRot, (width), (height) + 240, Integer.parseInt("FFFFFF", 16), false);
            }
            GL11.glPopMatrix();
        }
    }

    private void renderDevGuiBasic(Minecraft mc, ItemStack itemStack, GunType gunType, ModelGun gunModel, AnimStateMachine anim) {
        ScaledResolution scaled = new ScaledResolution(mc);
