...
public class RenderLayerBody implements LayerRenderer<EntityPlayer> {
    ...
    public void doRenderLayer(final EntityPlayer player, final float limbSwing, final float limbSwingAmount,
        ...
        if (player instanceof AbstractClientPlayer) {
            ItemStack gun = BackWeaponsManager.INSTANCE.getItemToRender((AbstractClientPlayer) player);
    ...
    public void renderBody(final EntityPlayer player, final ArmorType armorType, final float scale) {
        if (armorType.hasModel()) {
            ...
            final int skinId = 0;
            String path = skinId > 0 ? "skins/" + armorType.modelSkins[skinId].getSkin()
                    : armorType.modelSkins[0].getSkin();
            Minecraft.getMinecraft().getRenderManager().renderEngine
                    .bindTexture(new ResourceLocation(ModularWarfare.MOD_ID, "skins/armor/" + path + ".png"));
            GL11.glScalef(1.0f, 1.0f, 1.0f);
            
            ObjModelRenderer.glowType = "armor";
            ObjModelRenderer.glowPath = path;
