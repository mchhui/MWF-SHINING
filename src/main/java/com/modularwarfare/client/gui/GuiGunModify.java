...
public class GuiGunModify extends GuiScreen {
    ...
    private static final ProjectionHelper PROJECTION_HELPER = new ProjectionHelper();
    ...
    public GuiGunModify() {
        updateItemModifying();
        initGui();
        isEnglishLanguage = Minecraft.getMinecraft().getLanguageManager().getCurrentLanguage().getLanguageCode().contains("en_");
    }
    ...
    public void updateItemModifying() {
        ...
        boolean refresh = this.currentModify != null && !this.currentModify.equals(itemMainhand);
    ...
    public void initGui() {
        ...
        double scaleFactor = sFactor / scaledresolution.getScaleFactor();
        ...
        this.buttonList.add(new TextureButton(sideButtonIdOffset, 9999, 9999, buttonSize / 3, buttonSize, DECAL_1).setType(TypeEnum.SideButton));
    ...
    public void joinSubPageButtons(EntityPlayer player, ScaledResolution scaledresolution) {
        ...
        List<Integer> attachmentSlotList = checkAttach(mc.player, gunType, currentButton.getAttachmentType());
        int buttonSize = 90 / scaledresolution.getScaleFactor();
...
//        GlStateManager.shadeModel(7425);
        ...
        if (((GunType) type).animationType == WeaponAnimationType.BASIC) {
            renderBasicModel(entitylivingbaseIn, type, itemstack, sFactor, scaledresolution, f4);
        } else if (((GunType) type).animationType == WeaponAnimationType.ENHANCED) {
            renderEnhancedModel(entitylivingbaseIn, type, itemstack, sFactor, scaledresolution, f4);
        }
        // state fix
        ...
        PROJECTION_HELPER.updateMatrices();
        GlStateManager.popMatrix();
...
//					}
                    ...
                    if (itemStack != null && itemStack.getItem() != Items.AIR) {
                        ...
                        if (config.attachment.containsKey(attachmentType.internalName)) {
                            if ("gunModel".equals(config.attachment.get(attachmentType.internalName).binding)) {
                                if (config.attachmentGroup.containsKey(attachment.typeName)) {
                                    AttachmentGroup ag = config.attachmentGroup.get(attachment.typeName);
                                    if (ag != null) {
                                        partTranslate = new Vector3f(ag.translate);
...
//				}
                ...
                Vec3d partVec3d = PROJECTION_HELPER.project(partTranslate.x, partTranslate.y, partTranslate.z);
                ...
                GlStateManager.enableBlend();
                drawLine(scaledPartVec3d.x, scaledPartVec3d.y, nearestPoint[0] + (double) 8 / scaledresolution.getScaleFactor(), nearestPoint[1] + (double) 8 / scaledresolution.getScaleFactor());
                ...
                GlStateManager.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
                GlStateManager.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
                double topBGsize = (double) 30 / scaledresolution.getScaleFactor();
                RenderHelperMW.drawTexturedRect(nearestPoint[0] - ((double) 45 / scaledresolution.getScaleFactor()), nearestPoint[1] - ((double) 75 / scaledresolution.getScaleFactor()), topBGsize * 2, topBGsize);
                double iconSize = (double) 25 / scaledresolution.getScaleFactor();
                bindAttachmentTexture(attachment);
                RenderHelperMW.drawTexturedRect(nearestPoint[0] - ((double) 40 / scaledresolution.getScaleFactor()), nearestPoint[1] - (71.5 / scaledresolution.getScaleFactor()), iconSize, iconSize);
                ...
                for (GuiButton button : this.buttonList) {
                    if (button instanceof TextureButton) {
                        TextureButton tb = (TextureButton) button;
                        if (tb.getType().equals(TextureButton.TypeEnum.Slot) && tb.getAttachmentType() == attachment) {
                            tb.x = nearestPoint[0] - ((double) tb.width / 2);//*sFactor/scaledresolution.getScaleFactor()
                            tb.y = nearestPoint[1] - ((double) tb.height / 2);
                            tb.visible = true;
                            ...
                            sideButton.y = nearestPoint[1] - ((double) tb.height / 2);
                        }
                    }
                }
    ...
    public void drawSubPage(int mouseX, int mouseY, float partialTicks, double sFactor, ScaledResolution scaledresolution) {
        ...
        this.currentModify.getItem().addInformation(currentModify, null, toolTips, null);
        ...
        for (String str : toolTips) {
            ...
            RenderHelperMW.renderText(str, 2, (int) (indexY * 10) + 20, color);
            indexY++;
        }
        GlStateManager.popMatrix();
        //
    ...
    public void renderBasicModel(EntityLivingBase entitylivingbaseIn, BaseType type, ItemStack itemstack, double sFactor, ScaledResolution scaledresolution, float partialTicks) {
        ...
        double basicS = 300;
        double scale = basicS * model.config.extra.modelGuiScale * sFactor / scaledresolution.getScaleFactor();
        GlStateManager.translate(
                ((float) scaledresolution.getScaledWidth() / 2),
                ((float) scaledresolution.getScaledHeight() / 2), 0);
        ...
        for (AttachmentPresetEnum attachment : AttachmentPresetEnum.values()) {
            ...
            if (itemStack != null && itemStack.getItem() != Items.AIR) {
                ...
                for (GuiButton button : this.buttonList) {
                    TextureButton tb = (TextureButton) button;
                    if (tb.getType().equals(TypeEnum.Slot) && tb.isMouseOver() && tb.getAttachmentType() == attachment) {
                        drawEdge = true;
                    }
                }
//
                if (attachmentModel != null) {
                    ...
                    if (drawEdge) {//
                        ...
                        GlStateManager.disableTexture2D();
                        GlStateManager.disableDepth();
                        renderAttachModel(attachmentModel, attachment, model, attachmentType, worldScale, itemStack, path);
                        GlStateManager.popMatrix();
                        GlStateManager.enableDepth();
    ...
    public void renderAttachModel(ModelAttachment attachmentModel, AttachmentPresetEnum attachment, ModelGun model, AttachmentType attachmentType, float worldScale, ItemStack itemStack, String path) {
        ...
        if (model.config.attachments.attachmentPointMap != null
                && !model.config.attachments.attachmentPointMap.isEmpty()) {
    ...
    public void renderEnhancedModel(EntityLivingBase entitylivingbaseIn, BaseType type, ItemStack itemstack, double sFactor, ScaledResolution scaledresolution, float partialTicks) {
        ...
        if (config.animations.containsKey(AnimationType.DEFAULT)) {
            ...
            HashSet<String> exceptParts = new HashSet<>(config.defaultHidePart);
...
//             }
            ...
            double scale = 70 * config.extra.modelGuiScale * sFactor / scaledresolution.getScaleFactor();
            ...
            GlStateManager.translate(
                    ((float) scaledresolution.getScaledWidth() / 2),//+ (centerOffsetX * scaledresolution.getScaleFactor())
                    ((float) scaledresolution.getScaledHeight() / 2), 0);//+ (centerOffsetY * scaledresolution.getScaleFactor())
            GlStateManager.scale(scale, scale, -scale);
            ...
            rge.blendTransform(model, itemstack, false, rge.controller.getTime(),
                    ...
                    () -> {
                        ...
                        if (sightRendering != null) {
                            ...
                            model.applyGlobalTransformToOther(binding, () -> rge.renderAttachment(config, AttachmentPresetEnum.Sight.typeName,
                                    sightRendering.type.internalName, () -> rge.writeScopeGlassDepth(sightRendering.type,
                            ...
                            if (bulletStack.getItem() instanceof ItemBullet) {
                                ...
                                if (bulletType.isDynamicBullet && bulletType.model != null) {
                                    ...
                                    for (int bullet = 0; bullet < currentAmmoCount
                                        ...
                                        model.applyGlobalTransformToOther("bulletModel_" + bullet, () -> rge.renderAttachment(config, "bullet", bulletType.internalName, () -> bulletType.model.renderPart("bulletModel", worldScale)));
                                    }
                                    model.applyGlobalTransformToOther("bulletModel", () -> rge.renderAttachment(config, "bullet", bulletType.internalName, () -> bulletType.model.renderPart("bulletModel", worldScale)));
    ...
    public void updateRotate() {
        ...
        if (Mouse.getEventButton() == 0) {
            this.leftClicked = Mouse.getEventButtonState();
        }
        if (this.leftClicked) {
