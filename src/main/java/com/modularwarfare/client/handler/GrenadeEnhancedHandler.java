     ...
     public static void onTick(RenderTickEvent event) {
         ...
         if(lastSlot != currentSlot) {
             if(holdingStack != null && holdingStack.getItem() instanceof ItemGrenade) {
                 ...
                 if(!lastSlotStack.isEmpty() && lastSlotStack.getItem() instanceof ItemGrenade && isTimerStarted(lastSlotStack)) {
                     GrenadeType type = ((ItemGrenade)lastSlotStack.getItem()).type;
                     float remainingTime = type.fuseTime - (System.currentTimeMillis() - getTimerStartTime(lastSlotStack)) / 1000f;
                     ModularWarfare.NETWORK.sendToServer(new PacketGrenadeThrow(false, remainingTime, 0));
                     setTimerStarted(lastSlotStack, false);
                     lastSlotStack.shrink(1);
                     if(lastSlotStack.getCount() <= 0) {
                         mc.player.inventory.setInventorySlotContents(lastSlot, ItemStack.EMPTY);
                     }
                 }
                 isHolding = false;
                 isConsumed = false;
         ...
         if(!stack.isEmpty() && stack.getItem() instanceof ItemGrenade) {
             ...
             if(type != null && type.animationType==WeaponAnimationType.ENHANCED) {
                 ...
                 if(Mouse.isButtonDown(mc.gameSettings.keyBindAttack.getKeyCode()+100)) {
                     isThrowingTemp=true;
                 }
                 ...
                 if(isThrowingTemp && !isHolding) {
                     ...
                     if(machine != null && machine.throwing) {
                         if(machine.throwingPhase == EnhancedStateMachine.Phase.FIRST && 
                            ...
                            !isConsumed) {
                             if(stack.isEmpty() || stack.getCount() <= 0 || !(stack.getItem() instanceof ItemGrenade)) {
                                 isHolding = false;
                                 isConsumed = false;
                                 holdingStack = null;
                             ...
                             float remainingTime = isTimerStarted(stack) ? 
                                 type.fuseTime - (System.currentTimeMillis() - getTimerStartTime(stack)) / 1000f : -1;
                         ...
                         else if(machine.throwingPhase == EnhancedStateMachine.Phase.SECOND && 
                                machine.controller != null && 
                                machine.controller.GRENADE_THROW == 0) {
                             type.playClientSound(mc.player, WeaponSoundType.GrenadeThrowSecond);
                                ...
                                machine.controller.GRENADE_THROW >= 1.0 && 
                                isConsumed) {
                             type.playClientSound(mc.player, WeaponSoundType.GrenadeThrow);
                 ...
                 if(!isTimerStarted(stack) && !rKeyLock && Keyboard.isKeyDown(KeyType.GunReload.keyCode)) {
                     if(machine != null && machine.throwing && machine.throwingPhase != EnhancedStateMachine.Phase.PRE) {
                         rKeyLock = true;
